...
RSpec.describe 'Renders the post show page', type: :feature do
  before :each do
    @user = User.create(name: 'Bonita', photo: 'https://unsplash.com/photos/F_-0BxGuVvo', posts_counter: 0,
                        bio: 'developer')
    @first_post = Post.create(title: 'Hello', body: 'Post 1', comments_counter: 0, likes_counter: 0,
                              author_id: @user.id)
    5.times do |_i|
      @comment = Comment.create(text: 'thats great', author_id: @user.id, post_id: @first_post.id)
    end
  ...
  scenario 'I can see the comment each commentor left.' do
    expect(page).to have_content('thats great')
  end
end
