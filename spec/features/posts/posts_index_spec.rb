...
RSpec.describe 'Render posts index page', type: :feature do
  before :each do
    @user = User.create(name: 'pius', photo: 'https://unsplash.com/photos/F_-0BxGuVvo', posts_counter: 0,
                        bio: 'Bio 1')
    @first_post = Post.create(title: 'My post', body: 'Post 1', likes_counter: 0, comments_counter: 0,
                              author_id: @user.id)
    5.times do |_i|
      @comment = Comment.create(text: 'Post 1', author_id: @user.id, post_id: @first_post.id)
    end
